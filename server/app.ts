...
import { calculateBookingPrice } from "../shared/pricing.ts";
...
app.get("/api/stays/:stayId/reviews", (request, response) => {
  if (!stays.some((stay) => stay.id === request.params.stayId))
    return respondNotFound(response, "Stay");
...
app.post("/api/bookings", (request, response) => {
  // Ignore client price fields and rebuild the complete booking from validated intent.
  ...
  if (!stay.available || parsed.data.guests > stay.maxGuests) {
    const body: ApiError = {
      error: { code: "STAY_UNAVAILABLE", message: "This stay is not available for that party." },
    };
  ...
  // UTC date-only arithmetic keeps the night count stable across server time zones and DST.
  const nights = Math.round(checkOut.diff(checkIn, "days").days);
  if (nights < 1)
    return respondInvalid(response, { checkOut: ["Check-out must be after check-in"] });
